...
保存工作、停止游戏、关闭其他调参程序并解除既有锁频。点击“读取曲线”，选择驱动原有电压点并填写目标频率，再生成预览。低于锚点的曲线不变，锚点及右侧成为目标频率平台。低电压侧已高于目标、偏移越界、数据布局异常或范围不可读时拒绝应用。

safe.3 新增显式选择的 **4060 Laptop 保守试用预设**：使用本机 900 mV 点的原有频率，拒绝已有曲线偏移、错误型号、缺失锚点或需要提升任何曲线点的情况；不会自动套用。当前测试机器为 900 mV / 2250 MHz，不硬编码其他机器也适用的“稳定频率”。预设只做预览，后续仍需风险确认及 60 秒试用。它不改显存超频、不提升电压、不修改 CPU。

显存固定锁频改为驱动 `--query-supported-clocks=mem` 报告的离散选择，不再把通常相等的基础/Boost 值当上下限。界面和后端均拒绝列表外输入；读取失败不猜范围。读取档位不是写入能力探测，驱动仍可能拒绝锁频。`nvidia-smi` 输出包含失败/不支持或标准错误时，不因退出码 0 而误报成功。
...
safe.3 随后实测完整预设：54 个点变更，独立接口核对完整控制表、255 偏移及 900 mV 右侧 2250 MHz 平台，两次应用分别由手动恢复和后端 60 秒计时恢复，原始控制表与 P0 核心/显存/电压设置核对通过。**不等于降压收益或长时间游戏稳定性通过**；没有测试驱动挂起或蓝屏恢复。仅开放单 NVIDIA GPU 实验路径；驱动拒绝、静默忽略或取整导致读回不匹配均为失败。偏移单位仅在驱动报告 −1000～+1000 MHz 的已验证范围签名下允许写入。
