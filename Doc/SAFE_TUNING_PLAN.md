...
- [ ] Review diffs, document limitations/recovery, commit and push only to the user's fork.
- [ ] User-authorized extension: package a Windows x64 self-contained portable build, verify its contents/checksum, and publish an experimental GitHub prerelease with deployment and rollback instructions. Do not run the app or install drivers.
