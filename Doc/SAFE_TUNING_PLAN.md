...
- [x] Final diff/package review, committed-source Windows x64 self-contained build, ZIP entry/hash verification, push to user's fork, and publish an explicitly experimental safe.3 prerelease after matching uploaded digests. No safe.1 assets, installed software, local configurations or system settings were altered.

Remaining limits: no long-duration game/stress testing, power/voltage savings guarantee, memory-lock hardware write verification, crash/driver-hang recovery certification or full upstream OEM-feature audit. Preset is a conservative trial starting point, not an established stable OC profile. Rollback: restore offsets first, exit, then switch to the previous directory; source may be reverted by commit.

Published [v10.13.25-safe.3](https://github.com/lyy0709/JiaolongControl/releases/tag/v10.13.25-safe.3), release ID 383582897, source/tag commit `78abaaf5f6e3cb1c88bbf97774b3467bf448968c`. Complete ZIP contains 303 files, 75,665,267 bytes; informational version embeds that commit. Rebuilt 28 backend / 29 frontend tests and typecheck passed, production publish passed (existing 28 backend warnings and frontend bundle-size warnings remain). Every archive entry matched the published directory by SHA-256; no configuration, crash dumps, native diagnostic logs, credentials or test harnesses were packaged. The independent hardware-test files remain only in ignored local bin/.

ZIP SHA-256: `64a78d5293812e2fdc54fb63557565df56fb8e37a53b7224ea9be494d75da1a0`. Both uploaded assets matched GitHub's server digests before publication. A fresh unauthenticated public ZIP download matched this hash afterward. Source branch and annotated tag were pushed without force. Preview server stopped; hardware workers exited; no curve recovery record or test tuning remained. This is a prerelease, not marked latest stable.
